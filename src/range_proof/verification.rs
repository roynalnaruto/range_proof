...
pub fn verify(
    ...
    let (w1_part, w2_part, w3_part) =
        compute_w1_w2_w3_evals(&domain, &evaluations.g, &evaluations.g_omega, &rho, &tau);
    ...
    let aggregate_poly_commitment = commitment_scheme::aggregate_commitments(
        vec![&commitments.g, &w_cap_commitment],
        aggregation_challenge,
    );
    let aggregate_value = commitment_scheme::aggregate_values(
        vec![&evaluations.g, &evaluations.w_cap],
        aggregation_challenge,
    );
...
fn compute_rho_relations(domain: &GeneralEvaluationDomain<Fr>, rho: &Fr) -> (Fr, Fr) {
    let n_as_ref = utils::as_ref(&Fr::from(domain.size() as u8));
    let one = Fr::one();
    let rho_n_minus_1 = rho.pow(&n_as_ref) - one;
